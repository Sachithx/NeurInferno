...
class ByteLMModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.lm = ByteLM(
            d_model=d_model, n_heads=n_heads, d_ff=d_ff, n_layers=n_layers, max_len=max_len
        )
...
def train_lm(
    ...
    print(
        f"LM data (excl={excl or 'none'}): {len(all_train)} train groups, {len(all_val)} val groups"
    )
    print(f"         {sum(len(g.messages) for g in all_train):,} total sequences")
    print(f"         arch: d_model={d_model} n_heads={n_heads} d_ff={d_ff} n_layers={n_layers}")

    train_dl = make_lm_dataloader(
        all_train, batch_size=batch_size, max_len=max_len, shuffle=True, num_workers=num_workers
    )
    val_dl = make_lm_dataloader(
        all_val, batch_size=batch_size, max_len=max_len, shuffle=False, num_workers=num_workers
    )
...
if __name__ == "__main__":
    ...
    p.add_argument("--exclude_protocol", default=None, help="Single Tier-2 protocol to exclude.")
    p.add_argument(
        "--exclude_protocols",
        nargs="+",
        default=None,
