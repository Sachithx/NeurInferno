...
# ── Held-out splits ──────────────────────────────────────────────────────────
# Structural validation protocols. "coap" is retained for compatibility with
# the original training configuration and is skipped when absent from data/protocols.
...
def split_tier2_protocols(
    ...
    Used only when --use_dynamic_val is passed.
...
def load_tier1_groups(
    ...
    val = [
        _make_group(Path(tier1_dir / v))
        for v in val_dirs
        if (tier1_dir / v / "messages.jsonl").exists()
    ]
    return train, val
    ...
    for proto_dir in sorted(tier2_dir.iterdir()):
        ...
        if name in val_protocols:
            ...
            for i in range(0, len(msgs), chunk_size):
                ...
                if chunk:
                    val_groups.append(
                        FormatGroup(format_id=f"{name}_{i // chunk_size:04d}", messages=chunk)
                    )
        else:
...
def collate_format_batch(
    ...
    for grp in groups:
        msgs = grp.messages
        sampled = rng.choices(msgs, k=n_msgs) if len(msgs) < n_msgs else rng.sample(msgs, n_msgs)
        xs, fts, bgs = [], [], []
        for m in sampled:
