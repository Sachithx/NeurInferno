...
def boundary_loss(
    ...
    if use_focal:
        return focal_bce_loss(logits_flat, targets_flat, gamma=gamma, pos_weight=pw.unsqueeze(0))
...
def compute_total_loss(
    ...
    if use_consistency_loss:
        l_co = consistency_loss(boundary_logits.squeeze(-1).sigmoid(), ft_labels, gap_mask)
    else:
        l_co = boundary_logits.sum() * 0.0  # zero, keeps graph alive

    l_total = weights.boundary * l_bd + weights.type_ * l_ty + weights.consistency * l_co
