...
from neurinferno.evaluation.ground_truth import LabeledMessage, load_labeled_messages
...
def _to_tensor(msgs: list[LabeledMessage], max_len: int = 512) -> torch.Tensor:
    ...
    for i, s in enumerate(seqs):
        length = min(s.size(0), L)
        x[0, i, :length] = s[:length]
    return x  # (1, N, L)
...
def run_lopo_evaluation(
    ...
    for proto in protocols:
        ...
        try:
            ...
            print(
                f"  [{proto}] P={metrics.precision:.3f}  R={metrics.recall:.3f}"
