...
from neurinferno.evaluation.lopo import _find_best_ckpt, run_lopo_evaluation, run_model_inference
...
def _sweep_from_pr_curves(pr_curves: dict) -> list[dict]:
    ...
    for tau in _TAU_SWEEP:
        ps, rs, f1s = [], [], []
        for _proto, data in pr_curves.items():
...
def eval_lopo(
    ...
    for proto in LOPO_PROTOCOLS:
        m = ours.get(proto)
        if m is None:
            rows.append({"protocol": proto, "P": "N/A", "R": "N/A", "FPR": "N/A", "F1": "N/A"})
            continue
        rows.append(
            {
                "protocol": proto,
...
def _eval_one_l4po_split(
    ...
    if proto_scores:
        for tau in _TAU_SWEEP:
            ps, rs, f1s = [], [], []
            for _proto, (sc_list, gt_list) in proto_scores.items():
                per_msg = [
                    compute_boundary_metrics([1 if s >= tau else 0 for s in sc], gt)
                    for sc, gt in zip(sc_list, gt_list, strict=True)
                ]
                agg = aggregate_metrics(per_msg)
...
def eval_l4po(
    ...
    split_dirs = sorted(
        d for d in l4po_ckpt_dir.iterdir() if d.is_dir() and not d.name.startswith(".")
    )
    if not split_dirs:
    ...
    for split_dir in split_dirs:
        ...
        ckpt = _find_best_ckpt(
            split_dir,
            preferred_name=_preferred_ckpt_name(label),
        )
        if ckpt is None:
...
def _print_summary(lopo: dict[str, BoundaryMetrics]) -> None:
    ...
    for proto in LOPO_PROTOCOLS:
        m = lopo.get(proto)
        if m:
            print(f"{proto:<12} {m.precision:>7.4f} {m.recall:>7.4f} {m.fpr:>7.4f} {m.f1:>7.4f}")
            total_f1 += m.f1
            n += 1
        else:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Evaluate LOPO and L4PO checkpoints.")
